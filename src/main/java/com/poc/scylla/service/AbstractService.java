...
public abstract  class AbstractService {
    ...
    public AbstractService(Executor pExecutor){
        executor = pExecutor;
    }
    ...
    protected DynamoDB getClusterClient(){
        if(clusterClient == null){
            ...
            URI node = URI.create(String.format("http://%s:%s",getHost(),getAlternatorPort()));
    ...
    protected DynamoDB getClient(String dbType) {
        if(dbType.equals("scylla")) {
            if (standaloneClient == null) {
                AwsClientBuilder.EndpointConfiguration endpoint = new AwsClientBuilder.EndpointConfiguration(
                        String.format("http://%s:%s", getHost(), getAlternatorPort()), "None");

                dbClient = AmazonDynamoDBClientBuilder.standard().withEndpointConfiguration(endpoint)
                        .withCredentials(new AWSStaticCredentialsProvider(new BasicAWSCredentials("None", "None"))).build();
                standaloneClient = new DynamoDB(dbClient);

                streamsClient = AmazonDynamoDBStreamsClientBuilder.standard().withEndpointConfiguration(endpoint)
                        .withCredentials(new AWSStaticCredentialsProvider(new BasicAWSCredentials("None", "None"))).build();

            }
        }else if(dbType.equals("dynamo")){
            if(standaloneClient == null){
            dbClient = AmazonDynamoDBClientBuilder.standard().
                    withCredentials(new AWSStaticCredentialsProvider(new BasicAWSCredentials(getAccessKey(),
                    getSecretKey()))).withRegion(getRegion()).build();
            standaloneClient = new DynamoDB(dbClient);

            streamsClient = AmazonDynamoDBStreamsClientBuilder.standard().  withCredentials(new AWSStaticCredentialsProvider(new BasicAWSCredentials(getAccessKey(),
                        getSecretKey()))).withRegion(getRegion()).build();
        }}
    ...
    protected void loadData(InsertRequest input){
        Table table ;
            if (input.isUseCluster() && input.getDbType().equals("scylla")) {
    ...
    protected void createTable(InsertRequest insertRequest){
        try {
            DynamoDB client = insertRequest.isUseCluster() ? getClusterClient():getClient(insertRequest.getDbType());
    ...
    public List getRecords(DataRequest request) {
        Table table = request.isUseCluster() ? getClusterClient().getTable(request.getTableName()) : getClient(request.getDbType()).getTable(request.getTableName());
    ...
    @Async
    public void streams(String table,String dbType){
        getClient(dbType);
        ...
        do {
            ...
            for (Shard shard : shards) {
                ...
                while (currentShardIter != null && processedRecordCount < 10) {
                    ...
                    List<com.amazonaws.services.dynamodbv2.model.Record> records = getRecordsResult.getRecords();
                    if(records.size() > 0){
    ...
    abstract int getAlternatorPort();
